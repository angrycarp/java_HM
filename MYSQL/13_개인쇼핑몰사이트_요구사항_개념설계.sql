...
- 동일 제품코드와 옵션이 동일한 제품을 장바구니에 담는 경우 최근에 담은 수량으로 변경된다.
- 회원은 장바구니에 담긴 제품 목록을 선택해서 바로 구매할 수 있다.
- 회원은 장바군에 담긴 제품을 장바구니에서 제거할 수 있다.
...
- 제품 구매 후 구매확정이 되면 구매 금액의 10%가 마일리지로 적립이 된다.
- 제품 구매시 마일리지를 사용 할 수 있다.
...
두 개체의 관계가 1:n인 경우 - 관계에 속성이 없는

*/

/*
개인이 운영하는 쇼핑몽 사이트 구축을 위한 논리적 설계
회원(아이디(PK), 비번, 전화번호, 주소, 상세주소, 우편번호, 마일리지, 권한, 이름)
배송지(배송지번호(PK), 배송지명, 주소, 상세주소, 우편번호, 아이디(FK))
제품(제품코드(PK), 제품명, 상세설명, 가격)
옵션(옵션번호(PK), 옵션명, 수량)
제품이미지(이미지번호(PK), 파일명)
주문목록(주문목록번호(PK), 수량, 총가격)
마일리지내역(마일리지내역번호(PK), 내용, 마일리지, 아이디(FK))
주문(주문번호(PF), 총금액, 결제금액, 사용마일리지, 적립마일리지, 상태, 아이디(FK), 배송지번호(FK), 주문목록번호(FK))
