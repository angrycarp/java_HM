...
as : 속성 또는 테이블명을 일시적으로 변경해서 사용할 때 쓰는 키워들
*/
use course;
...
or : ~하거나, 자바의 ||연산자
is null : null 인지 확인
is not null : null 이 아닌지 확인
like : 와일드카드와 함께 사용되며, 값이 주어진 와이드가 포함됨 문자열과 같은지를 비교할 때 사용
_ : 한글자를 의미하는 와일드 카드
& : 0글자이상을 의미하는 와일드 카드
속성명 between A and B : 속성의 값이 A이상 B이하인 조건을 만족할 때 사용
=> 속성명 >= A and 속성명 <= B
속성명 in(값1, 값2, ..., 값n) : 속성의 값이1이거나 값2이거나 ...값n인 
*/
/*
...
select * from subject where point >= 2 && point <= 3; 
-- 이순신교수와 한석봉교수가 강의하는 수강정보를 조회하는 쿼리
