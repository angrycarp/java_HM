...
=> 데이터 일관성을 유지하기 위해
트리거 예시
- 쇼핑몰에서 회원이 주문을 완료하면 주문테이블에 데이터가 추가 => 옵션 수량이 변경 
트리거 특징
...
DELIMITER //
CREATE TRIGGER DELETE_BOARD AFTER INSERT ON BOARD
FOR EACH ROW
BEGIN
	UPDATE MEMBER
    SET
...
INSERT INTO
`ORDER`(OR_TOTAL, OR_PRICE, OR_USE_POINT, OR_SAVE_POINT, OR_ME_ID, OR_AD_NUM)
VALUES(45000, 45000, 0, 4500, 'qwe123', 2);
...
UPDATE `ORDER` SET OR_LIST = '구매확정' WHERE OR_NUM = 3;
...
BEGIN
	UPDATE `POINT`
    SET
		PO_AMOUNT = PO_AMOUNT + NEW.PO_AMOUNT
	WHERE
		OR_ME_ID = NEW.OR_ME_ID;
END //
DELIMITER ;